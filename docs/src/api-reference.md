...
> **A note on language.** The prose guide (this book) is available in English and
> Japanese. The API reference itself is generated by `rustdoc`, which has no
> language-switching mechanism, so it is **English only**. The guide's
> [Feature Tour](./feature-tour.md) and [Key Concepts](./concepts.md) chapters
> are the Japanese-language entry points into the same material.

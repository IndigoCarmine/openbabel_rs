//! Integration tests for the residue subsystem (T9).

use openbabel::Molecule;

/// A single-residue chlorinated-amine ligand (`00T`), taken from OpenBabel's
/// own test suite. Read as PDB it yields one residue on chain A, with explicit
/// hydrogens and 3D coordinates.
const LIGAND_PDB: &str = r#"ATOM      1  N19 00T A   1      -2.126  -2.241  -0.764  1.00 10.00           N
ATOM      2  C10 00T A   1      -1.652  -1.628   0.484  1.00 10.00           C
ATOM      3  C6  00T A   1      -0.631  -0.566   0.166  1.00 10.00           C
ATOM      4  C5  00T A   1      -1.043   0.686  -0.251  1.00 10.00           C
ATOM      5  C32 00T A   1      -2.513   0.988  -0.389  1.00 10.00           C
ATOM      6  N33 00T A   1      -3.078   1.283   0.934  1.00 10.00           N
ATOM      7  C4  00T A   1      -0.107   1.661  -0.542  1.00 10.00           C
ATOM      8  C3  00T A   1       1.241   1.384  -0.416  1.00 10.00           C
ATOM      9  C2  00T A   1       1.654   0.131   0.002  1.00 10.00           C
ATOM     10 CL1  00T A   1       3.348  -0.217   0.160  1.00 10.00          CL
ATOM     11  C7  00T A   1       0.717  -0.842   0.298  1.00 10.00           C
ATOM     12 HN19 00T A   1      -1.355  -2.609  -1.301  1.00 10.00           H
ATOM     13  H10 00T A   1      -2.494  -1.177   1.010  1.00 10.00           H
ATOM     14 H10A 00T A   1      -1.197  -2.392   1.115  1.00 10.00           H
ATOM     15  H32 00T A   1      -2.647   1.850  -1.043  1.00 10.00           H
ATOM     16 H32A 00T A   1      -3.023   0.124  -0.818  1.00 10.00           H
ATOM     17 HN33 00T A   1      -2.580   2.039   1.379  1.00 10.00           H
ATOM     18 HN3A 00T A   1      -4.064   1.489   0.868  1.00 10.00           H
ATOM     19  H4  00T A   1      -0.429   2.638  -0.868  1.00 10.00           H
ATOM     20  H3  00T A   1       1.972   2.146  -0.644  1.00 10.00           H
ATOM     21  H7  00T A   1       1.039  -1.819   0.629  1.00 10.00           H
ATOM     22  H2  00T A   1      -2.812  -2.957  -0.577  1.00 10.00           H
END
"#;

#[test]
fn parses_single_residue() {
    let mol = Molecule::parse(LIGAND_PDB, "pdb").expect("parse PDB");
    assert_eq!(mol.num_residues(), 1);

    let res = mol.residue(0).expect("residue 0 exists");
    assert_eq!(res.index(), 0);
    assert_eq!(res.name(), "00T");
    assert_eq!(res.chain(), "A");
    assert_eq!(res.number(), 1);
    // 11 heavy atoms (N, 8×C, Cl grouped as heavy) + 11 explicit hydrogens.
    assert_eq!(res.num_atoms(), mol.num_atoms());
    assert_eq!(res.num_heavy_atoms(), 11);
    assert_eq!(res.atoms().len(), mol.num_atoms() as usize);
}

#[test]
fn atom_residue_backlink() {
    let mol = Molecule::parse(LIGAND_PDB, "pdb").expect("parse PDB");

    let atom = mol.atom(0).expect("atom 0 exists");
    let res = atom.residue().expect("atom 0 belongs to a residue");
    assert_eq!(res.name(), "00T");

    // First record is N19: an ATOM (not HETATM), serial 1, atom-id "N19".
    assert!(!atom.is_hetatm());
    assert_eq!(atom.serial_number(), 1);
    assert_eq!(atom.residue_atom_id().trim(), "N19");
}

#[test]
fn residues_iterate() {
    let mol = Molecule::parse(LIGAND_PDB, "pdb").expect("parse PDB");
    let names: Vec<String> = mol.residues().map(|r| r.name()).collect();
    assert_eq!(names, vec!["00T".to_string()]);
    // Out-of-range index yields None.
    assert!(mol.residue(1).is_none());
}

#[test]
fn smiles_molecule_has_no_residues() {
    let mol = Molecule::parse("CCO", "smi").expect("parse SMILES");
    assert_eq!(mol.num_residues(), 0);
    assert!(mol.residue(0).is_none());
    // Atoms of a residue-less molecule report no residue.
    assert!(mol.atom(0).expect("atom 0").residue().is_none());
}
